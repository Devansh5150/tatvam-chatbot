...
export default function WhatTatvamIsNot() {
  return (
    <section className="px-6 md:px-12 py-28 md:py-36 overflow-hidden relative">
      ...
      <div className="absolute top-0 right-0 w-80 h-80 rounded-full blur-[130px] -translate-y-20 translate-x-20 pointer-events-none"
        style={{ background: 'rgba(122,26,26,0.12)' }} />
      <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-[500px] h-32 blur-[80px] pointer-events-none"
        style={{ background: 'rgba(122,26,26,0.06)' }} />

      <div className="max-w-4xl mx-auto space-y-16 relative z-10">

        {/* Heading */}
        <div className="space-y-5 text-center">
          <p className="font-tiro text-[#C06060] text-lg tracking-[0.2em] opacity-80">
            सीमा का बोध
          </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
          ...
          <div className="space-y-3">
            {points.map((point, idx) => (
              <div key={idx}
                className="flex items-center gap-4 p-5 rounded-2xl border border-white/5 group
                           hover:border-[#7A1A1A]/30 transition-all duration-300 backdrop-blur-sm"
                style={{ background: 'linear-gradient(135deg, rgba(12,9,5,0.9), rgba(18,8,8,0.85))' }}>
                {/* Crossed chakra icon */}
                <div className="w-8 h-8 rounded-full border border-[#7A1A1A]/40 flex items-center justify-center
                                flex-shrink-0 group-hover:bg-[#7A1A1A]/15 transition-colors">
          ...
          <div className="relative group">
            <div className="absolute -inset-px rounded-[2.5rem] pointer-events-none"
              style={{ background: 'linear-gradient(135deg, rgba(192,96,96,0.2), transparent, rgba(122,26,26,0.1))' }} />
            <div className="relative p-10 rounded-[2.5rem] border border-[#7A1A1A]/20 group-hover:border-[#7A1A1A]/40
              ...
              {/* Sanskrit echo */}
              <p className="font-tiro text-[#7A1A1A]/50 text-sm tracking-widest">
